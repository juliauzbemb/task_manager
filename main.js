...
;// CONCATENATED MODULE: ./src/js/tasks/task.js
class Task {
  constructor(task) {
    this.task = task;
    this.isPinned = false;
...
;// CONCATENATED MODULE: ./src/js/tasks/task_manager.js
...
class TaskManager {
  constructor() {
    ...
    this.containerAll = document.querySelector(".tasks_all").querySelector(".tasks_items");
    ...
    this.containerPinned = document.querySelector(".tasks_pinned").querySelector(".tasks_items");
  ...
  filterTasks(search) {
    const result = this.storage.filter(item => item.task.toLowerCase().includes(search) && item.isPinned === false);
    return result;
  }
  addToStorage(task) {
  ...
  handleTaskOnClick(e, containerCheck, classListAction, isPinned, containerForInsertion) {
    let taskTarget;
    if ([...containerCheck.querySelectorAll(".tasks_item")].includes(e.target)) {
      taskTarget = e.target;
    } else {
      taskTarget = e.target.closest(".tasks_item");
    }
    let checkElement = taskTarget.querySelector(".tasks_check");
    let taskTextContent = taskTarget.querySelector(".tasks_text").textContent.trim();
    ...
    this.setMessageVisibility([...this.containerPinned.querySelectorAll(".tasks_item")], this.no_pinned);
    this.setMessageVisibility([...this.containerAll.querySelectorAll(".tasks_item")], this.not_found);
  }
  onClickContainerPinned(e) {
    this.handleTaskOnClick(e, this.containerPinned, "remove", false, this.containerAll);
  }
  onClickContainerAll(e) {
    this.handleTaskOnClick(e, this.containerAll, "add", true, this.containerPinned);
  }
}
